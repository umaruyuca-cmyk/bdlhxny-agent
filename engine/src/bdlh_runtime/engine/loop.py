...
def _assemble_messages(*, system_prompt: str, turn: AgentTurn, history_turns: int) -> list[Any]:
    ...
    if turn.context_degraded:
        messages.append(SystemMessage(content="固定上下文不可用（context_degraded），不得编造缺失事实。"))
    elif turn.context_items:
        lines = [item for item in turn.context_items if item]
        if lines:
            body = "\n".join(f"- {line}" for line in lines)
            messages.append(SystemMessage(content=f"固定用例上下文：\n{body}"))
